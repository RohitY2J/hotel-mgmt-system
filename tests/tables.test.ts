...
test.describe('Room Component E2E Tests', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto(`${testEnv.FRONTEND_URL}/admin/tables`, { waitUntil: 'networkidle' });
  });
   ...
   */
  test('should create a list new rooms successfully', async ({ page }) => {
    ...
    //   maintainanceStatus: Math.floor(Math.random() * 2).toString()
      ...
      id : "",
      location: "in the middle of room",
      status: Math.floor(Math.random() * 1).toString(),
      tableNumber: `TT-${i}`
    }));
    ...
    //   } else {
        ...
        // Fill form fields
        await page.fill('input[formControlName="tableNumber"]', table.tableNumber);
        await page.fill('input[formControlName="location"]', table.location);
        await page.selectOption('select[formControlName="status"]', table.status);
        await page.fill('input[formControlName="capacity"]', table.capacity.toString());
        ...
        console.log(`Table ${table.tableNumber} created successfully`);
      //}
    }
  });
